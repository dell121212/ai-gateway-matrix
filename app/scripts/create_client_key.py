...
def load_env() -> None:
    ...
    import os
    if os.environ.get("AI_GATEWAY_HOME"):
        path = Path(os.environ["AI_GATEWAY_HOME"]).expanduser() / ".env"
    elif (code.parent / "run.sh").is_file() and (code.parent / "home" / ".env").is_file():
        path = code.parent / "home" / ".env"
    else:
        path = code / ".env"
