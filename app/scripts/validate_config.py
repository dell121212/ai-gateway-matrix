...
# 代码根（gateway/scripts/…）；用户数据：AI_GATEWAY_HOME 或 仓库 home/。
CODE_ROOT = Path(__file__).resolve().parents[1]
ROOT = CODE_ROOT  # 兼容旧引用


def _default_data_root(code_root: Path) -> Path:
    env = (os.environ.get("AI_GATEWAY_HOME") or "").strip()
    if env:
        return Path(env).expanduser().resolve()
    # 源码布局：repo/{run.sh, app/, home/}
    repo_home = code_root.parent / "home"
    if (code_root / "gateway").is_dir() and (code_root.parent / "run.sh").is_file():
        return repo_home.resolve()
    return code_root.resolve()


DATA_ROOT = _default_data_root(CODE_ROOT)
