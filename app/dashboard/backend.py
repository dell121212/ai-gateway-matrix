...
async def lifespan(_app: FastAPI):
    # Professional backend bootstrap (schema / admin / key migration). Soft-fail.
    try:
        from dashboard.app.main_mount import run_bootstrap_safe

        await run_bootstrap_safe()
...
app = FastAPI(
    ...
    docs_url="/api/v1/openapi.json" if os.environ.get("DASHBOARD_AUTH") == "accounts" else None,
    redoc_url=None,
...
# accounts：专业用户体系（Argon2 + 会话 Cookie + RBAC）。
...
try:
    ...
    mount_professional_api(app)
except Exception as exc:  # pragma: no cover
    logger.warning("professional API mount failed: %s", exc)
...
async def proxy_litellm(request: Request, proxy_path: str):
    ...
    is_chat = request.method == "POST" and (
        path_norm.endswith("v1/chat/completions")
        or path_norm == "v1/chat/completions"
        or path_norm.endswith("chat/completions")
    )

    if is_chat and request.method == "POST":
        body_bytes = await request.body()
        try:
            body_obj = _json.loads(body_bytes.decode("utf-8") or "{}")
        except Exception:
            body_obj = {}
        ...
        try:
            ...
            async with factory() as session:
                # local default admin user if any
                ur = await session.execute(
                    _select(_User).where(_User.role == "super_admin").limit(1)
                )
